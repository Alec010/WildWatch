...
export default function ReportScreen() {
  ...
  const doSubmit = async () => {
    ...
    // Headers - Android needs special handling
    const headers: any = {
      Authorization: `Bearer ${token}`,
    ...
    try {
      ...
      if (error.name === "AbortError" || error.message?.includes("timeout")) {
        ...
        throw new Error("Network error. Please check your internet connection and try again. If using Expo Go, ensure you're on the same network as your development server.");
  ...
  const requestMediaLibraryPermissions = async () => {
    try {
      ...
      Alert.alert(
        "Permission Error",
        error?.message || "Failed to request photo library permissions. Please try again or enable it in settings.",
        [
          { text: "OK" },
          {
            text: "Open Settings",
  ...
  const takePhoto = async () => {
    try {
      ...
      Alert.alert(
        "Error",
        error?.message || "Failed to take photo. Please check your camera permissions and try again.",
        [{ text: "OK" }]
      );
    }
  };
  ...
  const pickImages = async () => {
    try {
      ...
      try {
        ...
        console.error("Error checking media library permission:", permissionError);
        Alert.alert(
          "Permission Error",
      ...
      Alert.alert(
        "Error",
        error?.message || "Failed to pick images. Please check your photo library permissions and try again.",
        [{ text: "OK" }]
      );
    }
  };
