...
export default function MapPickerModal({
  ...
  // Show help dialog when map modal opens if no location is selected
  useEffect(() => {
    if (!isOpen) {
    ...
    // Show help dialog if no location is selected yet
    if (!selectedLocation) {
      // Small delay to ensure map modal is fully rendered
      const timer = setTimeout(() => {
        setShowHelpDialog(true);
      }, 500);
      return () => clearTimeout(timer);
    } else {
      // If location is already selected, don't show help dialog
      setShowHelpDialog(false);
    }
    ...
    async (lat: number, lng: number) => {
      ...
      try {
        ...
        toast.error("Failed to process location", {
          description:
            error.message || "Please try selecting a different location.",
        });
      } finally {
  ...
  return (
    <>
      <Dialog open={isOpen} onOpenChange={handleClose}>
        <DialogContent className="max-w-4xl max-h-[90vh] flex flex-col overflow-hidden">
          <DialogHeader className="flex-shrink-0">
            ...
            <DialogDescription>
              Click anywhere on the map to select a location. Building
              boundaries are highlighted in red.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="flex flex-col flex-1 overflow-hidden">
            ...
            <div className="relative flex-1 min-h-[500px]">
              ...
              <Button
                ...
                className="absolute top-4 right-4 bg-white hover:bg-gray-50 text-[#800000] border border-gray-300 shadow-md z-10"
                size="sm"
              >
                {isProcessingLocation ? (
              ...
              {/* Overlay Cards Container - Positioned inside map */}
              {selectedLocation && (
                <div className="absolute top-4 left-4 z-10 max-w-sm space-y-3 pointer-events-none">
                  {/* Selected Location Display - Overlay inside map */}
                  {(() => {
                    const isOutsideCampus =
                      selectedLocation.withinCampus === false;
                    return (
                      <div className="pointer-events-auto">
                        <div
                          ...
                          } border rounded-lg p-4 shadow-lg backdrop-blur-sm bg-opacity-95`}
                        >
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              {isOutsideCampus ? (
                                <div className="flex items-center gap-2 mb-1">
                                  <Building className="h-4 w-4 text-red-600 flex-shrink-0" />
                  ...
                  {/* Room/Location Input Field - Overlay inside map, positioned below Selected Location */}
                  <div className="pointer-events-auto">
                    <div className="bg-white border border-gray-200 rounded-lg p-4 shadow-lg backdrop-blur-sm bg-opacity-95">
                      <Label
                        htmlFor="room"
      ...
      {/* Help Dialog - Instructions on how to select location */}
      <Dialog open={showHelpDialog} onOpenChange={setShowHelpDialog}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Follow these steps to select a location on the map
            </DialogDescription>
          </DialogHeader>
          <div className="py-4">
            <div className="space-y-4">
              <div className="space-y-3">
                <div className="flex items-start gap-3 text-sm text-gray-700">
                  <div className="mt-0.5 bg-[#800000]/10 p-1.5 rounded-full text-[#800000] flex-shrink-0">
                    <MapPin className="h-4 w-4" />
                  </div>
                  <div>
                    <span className="font-medium">Click on the map:</span> Click
                    anywhere within the{" "}
                    <span className="font-semibold text-[#800000]">
                      red campus boundary
                    </span>{" "}
                    to place a marker and select a location.
                  </div>
                </div>
                <div className="flex items-start gap-3 text-sm text-gray-700">
                  <div className="mt-0.5 bg-[#800000]/10 p-1.5 rounded-full text-[#800000] flex-shrink-0">
                  ...
                  <div>
                    <span className="font-medium">Building boundaries:</span>{" "}
                    The{" "}
                    <span className="font-semibold text-[#800000]">
                      red outline
                    </span>{" "}
                    shows the CITU campus area. Building boundaries are
                ...
                <div className="flex items-start gap-3 text-sm text-gray-700">
                  ...
                  <div>
                    <span className="font-medium">Use zoom and pan:</span> Use
                    your mouse or touch gestures to zoom in/out and pan around
                    the map to find the exact spot.
                  </div>
                </div>
                <div className="flex items-start gap-3 text-sm text-gray-700">
                  <div className="mt-0.5 bg-[#800000]/10 p-1.5 rounded-full text-[#800000] flex-shrink-0">
                  ...
                  <div>
                    ...
                    Click the navigation button in the top-right corner to
