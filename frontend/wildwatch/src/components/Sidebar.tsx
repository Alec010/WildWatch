...
export function Sidebar() {
  ...
  useEffect(() => {
    const loadUserProfile = async () => {
      // Only proceed if role is loaded
      if (roleLoading) return;

      // If no role, clear user state
      if (!userRole) {
        setUser(null);
        setLoading(false);
  ...
  // Memoize user display name to prevent flickering
  const userDisplayName = useMemo(() => {
    if (!user) return "";
    return `${user.firstName} ${user.lastName}`;
  }, [user?.firstName, user?.lastName]);

  // Memoize user ID to prevent flickering
                ...
                {loading ? (
                  ...
                  <>
                    ...
                    <div
                      ...
                      style={{ minHeight: "40px", transition: "none" }}
                    >
                      <div
                      ...
                      <div
                        className="text-xs text-[#D4AF37]/80 truncate max-w-[120px]"
                        style={{
                          lineHeight: "1rem",
                          transition: "none",
                          fontSize: "0.75rem",
